...
Novel Contribution Plots (Journal)
-----------------------------------
11. AURC Risk-Coverage curve  – calibration quality (AUGT — NC2)
12. Pseudo-Label distribution – PLP assignment stats (NC3)
13. GNNExplainer Fidelity     – Fidelity+/− bar chart (NC4)
14. Heterophily score dist    – fraud vs licit embedding divergence
"""
...
# ─── Panel 11: AURC Risk-Coverage curve (AUGT — NC2) ─────────────────────────

def plot_aurc(aurc_coverages, aurc_risks, aurc_value, ax):
    """
    Plot the Risk-Coverage curve for AUGT.

    Lower area (AURC) = better calibrated model: when the model is confident,
    it is also correct.

    Parameters
    ----------
    aurc_coverages : (T,) array of coverage fractions [0, 1]
    aurc_risks     : (T,) array of risk values (1 - precision)
    aurc_value     : float scalar AURC
    """
    ax.fill_between(aurc_coverages, aurc_risks, alpha=0.25, color="#e74c3c")
    ax.plot(aurc_coverages, aurc_risks, color="#e74c3c", linewidth=2.0,
            label=f"ElliGAT  AURC = {aurc_value:.4f}")
    ax.set_xlim(0, 1.02)
    ax.set_ylim(0, 1.02)
    ax.legend(fontsize=9)
    _ax_style(ax, "Risk-Coverage Curve (AUGT — NC2)",
              "Coverage (fraction classified)", "Risk (1 − Precision)")


# ─── Panel 12: Pseudo-Label distribution (PLP — NC3) ─────────────────────────

def plot_pseudo_labels(pseudo_mask, pseudo_y, total_unlabelled, ax):
    """
    Bar chart showing pseudo-label assignment stats from PLP.

    Parameters
    ----------
    pseudo_mask      : (N,) bool tensor/array — nodes that got pseudo-labels
    pseudo_y         : (N,) float tensor/array — 0.0 = licit, 1.0 = fraud
    total_unlabelled : int — total number of unlabelled nodes in the graph
    """
    try:
        pm = pseudo_mask.cpu().numpy() if hasattr(pseudo_mask, 'cpu') else np.asarray(pseudo_mask)
        py = pseudo_y.cpu().numpy() if hasattr(pseudo_y, 'cpu') else np.asarray(pseudo_y)
    except Exception:
        pm = np.asarray(pseudo_mask)
        py = np.asarray(pseudo_y)

    n_pseudo_licit = int(((pm) & (py == 0)).sum())
    n_pseudo_fraud = int(((pm) & (py == 1)).sum())
    n_uncertain    = total_unlabelled - n_pseudo_licit - n_pseudo_fraud

    labels = ["Pseudo-Licit", "Pseudo-Fraud", "Uncertain\n(no label)"]
    values = [n_pseudo_licit, n_pseudo_fraud, n_uncertain]
    ...
    total_pseudo = n_pseudo_licit + n_pseudo_fraud
    pct = 100 * total_pseudo / max(total_unlabelled, 1)
    _ax_style(ax,
              f"Pseudo-Label Propagation (PLP — NC3)\n"
              f"{total_pseudo:,} / {total_unlabelled:,} labelled ({pct:.1f}%)",
              "", "# Nodes")


# ─── Panel 13: GNNExplainer Fidelity (NC4) ───────────────────────────────────

def plot_fidelity(expl_results, ax):
    """
    Bar chart of mean Fidelity+ and Fidelity− from GNNExplainer.

    Fidelity+ (high = good): how much does the prediction change when the
    important edges are removed?
    Fidelity− (low = good): how much does the prediction change when only
    the important edges are kept?

    Parameters
    ----------
    expl_results : dict from explain_fraud_nodes() with keys
                   fidelity_plus_mean/std, fidelity_minus_mean/std
    """
    labels = ["Fidelity+\n(higher = better)", "Fidelity−\n(lower = better)"]
    means  = [expl_results["fidelity_plus_mean"],
              expl_results["fidelity_minus_mean"]]
    stds   = [expl_results["fidelity_plus_std"],
              expl_results["fidelity_minus_std"]]
    colors = ["#e74c3c", "#3498db"]

    bars = ax.bar(labels, means, yerr=stds, color=colors, alpha=0.85,
                  capsize=8, edgecolor="white", error_kw={"linewidth": 1.5})
    for bar, m, s in zip(bars, means, stds):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + s + 0.01,
                f"{m:.3f}±{s:.3f}", ha="center", va="bottom", fontsize=9)

    n = expl_results.get("n_explained", 0)
    _ax_style(ax, f"GNNExplainer Fidelity (NC4, n={n} fraud nodes)", "", "Score")
    ax.set_ylim(0, max(means) * 1.5 + 0.1)


# ─── Panel 14: Heterophily score distribution ─────────────────────────────────

def plot_heterophily(hetero_scores, y_labels, ax):
    """
    Histogram of heterophily scores (||h_i − mean(h_N)||) split by fraud/licit.

    High scores indicate nodes whose embeddings diverge from their neighbours —
    the signature of fraud in a heterophilous graph.

    Parameters
    ----------
    hetero_scores : (N,) float array — per-node heterophily L2 scores
    y_labels      : (N,) int array   — 0 = licit, 1 = fraud
    """
    try:
        hs = hetero_scores.cpu().numpy() if hasattr(hetero_scores, 'cpu') else np.asarray(hetero_scores)
        yl = y_labels.cpu().numpy() if hasattr(y_labels, 'cpu') else np.asarray(y_labels)
    except Exception:
        hs = np.asarray(hetero_scores)
        yl = np.asarray(y_labels)

    licit_scores = hs[yl == 0]
    fraud_scores = hs[yl == 1]

    ax.hist(licit_scores, bins=50, alpha=0.55, color="#2ecc71",
            label=f"Licit (μ={licit_scores.mean():.2f})", density=True)
    ax.hist(fraud_scores, bins=50, alpha=0.70, color="#e74c3c",
            label=f"Fraud (μ={fraud_scores.mean():.2f})", density=True)
    ax.legend(fontsize=9)
    _ax_style(ax,
              "Heterophily Score Distribution\n"
              "||h_i − mean(h_N)|| (higher = more anomalous)",
              "Heterophily Score (L2)", "Density")
...
def save_results_figure(
    ...
    # Optional extras for existing panels
    timestep_results: dict | None = None,
    tab_model=None,
    feature_names: list | None = None,
    test_timesteps: np.ndarray | None = None,
    test_amounts: np.ndarray | None = None,
    # Novel contribution data (journal)
    aurc_data: dict | None = None,
    pseudo_data: dict | None = None,
    expl_results: dict | None = None,
    hetero_data: dict | None = None,
):
    """
    Compose multi-panel figure and save to disk.

    Base panels (always rendered): 1–7 + summary table  (rows 0–1)
    ...
    Novel Contribution panels (row 3, rendered when data provided):
        Panel 11 – AURC risk-coverage curve (requires aurc_data)
        Panel 12 – Pseudo-label distribution (requires pseudo_data)
        Panel 13 – GNNExplainer fidelity    (requires expl_results)
        Panel 14 – Heterophily score dist   (requires hetero_data)
    """
    ...
    has_aurc    = aurc_data is not None
    has_pseudo  = pseudo_data is not None
    has_expl    = (expl_results is not None
                   and expl_results.get("available")
                   and expl_results.get("n_explained", 0) > 0)
    has_hetero  = hetero_data is not None

    n_extra_row2 = sum([has_timestep, has_fimp, has_error])
    n_extra_row3 = sum([has_aurc, has_pseudo, has_expl, has_hetero])
    ...
    fig.suptitle(
        "Bitcoin Fraud Detection — ElliGAT-J (Journal)\n"
        "Novel: FAGA · AUGT · PLP · GNNExplainer",
    ...
    for name in ORDER:
        if name not in aggs:
            continue
        a = aggs[name]
        aurc_col = (f"{a['AURC'][0]:.4f}" if "AURC" in a else "n/a")
        rows.append([
            name,
    ...
    if rows:
        tbl = ax.table(
            cellText=rows,
            colLabels=["Model", "AUC", "F1", "MCC", "AURC↓"],
    ...
    # ── Row 2 (extra analysis panels — existing) ──────────────────────────
    row_idx = 2
    if n_extra_row2 > 0:
        extra_idx = 0
        if has_timestep:
            plot_timestep_f1(timestep_results, axes[row_idx, extra_idx])
            extra_idx += 1
        if has_fimp:
            plot_feature_importance(
                tab_model, feature_names,
                axes[row_idx, extra_idx],
            )
            extra_idx += 1
        if has_error:
    ...
    # ── Row 3 (Novel Contribution panels — JOURNAL) ───────────────────────
    if n_extra_row3 > 0:
        novel_idx = 0

        if has_aurc:
            plot_aurc(
                aurc_data["coverages"], aurc_data["risks"],
                aurc_data["aurc"], axes[row_idx, novel_idx],
            )
            novel_idx += 1

        if has_pseudo:
            plot_pseudo_labels(
                pseudo_data["mask"], pseudo_data["y"],
                pseudo_data["total_unlabelled"],
                axes[row_idx, novel_idx],
            )
            novel_idx += 1

        if has_expl:
            plot_fidelity(expl_results, axes[row_idx, novel_idx])
            novel_idx += 1

        if has_hetero:
            plot_heterophily(
                hetero_data["scores"], hetero_data["labels"],
                axes[row_idx, novel_idx],
            )
            novel_idx += 1

        for j in range(novel_idx, n_cols):
            axes[row_idx, j].axis("off")
