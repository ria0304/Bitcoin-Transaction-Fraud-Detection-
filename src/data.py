...
def find_dataset_path(dataset_name: str, project_data_path: str, cache_path: str) -> str:
    ...
    for path in candidates:
        if not os.path.isdir(path):
            print(f"  [check] not a directory: {path}")
            continue
        if os.path.exists(os.path.join(path, "elliptic_txs_features.csv")):
            print(f"  [check] found dataset at: {path}")
            return path
        ...
        for sub in os.listdir(path):
            ...
            if os.path.isdir(full) and os.path.exists(
                ...
                print(f"  [check] found dataset one level under: {full}")
                return full
        if not found_sub:
            print(f"  [check] directory exists but no elliptic_txs_features.csv (checked {path} and its subfolders)")
    print("  Dataset not found locally. Attempting download via KaggleHub …")
    downloaded = _ensure_dataset(dataset_name)
    if downloaded is None:
        return None
    if os.path.exists(os.path.join(downloaded, "elliptic_txs_features.csv")):
        return downloaded
    # KaggleHub sometimes nests the CSVs one level deeper, e.g.
    # .../versions/1/elliptic_bitcoin_dataset/elliptic_txs_features.csv
    for sub in os.listdir(downloaded):
        full = os.path.join(downloaded, sub)
        if os.path.isdir(full) and os.path.exists(
            os.path.join(full, "elliptic_txs_features.csv")
        ):
            return full
