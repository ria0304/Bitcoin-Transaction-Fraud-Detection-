...
• Phase-3 pseudo-label propagation (PLP) via src/pseudo_label.py
• FAGA-powered contrastive loss (genuine two-view contrastive signal)
...
• AUGT: Adaptive Uncertainty-Gated Threshold replacing static grid search;
  produces a deployable decision rule without threshold leakage
• AURC: Area Under Risk-Coverage Curve added to evaluate_gnn() and
  compute_metrics() as a calibration quality metric
• MC Dropout uncertainty used for AUGT threshold fitting and PLP
"""
...
from src.losses import CombinedLoss, CombinedASLLoss, FocalLoss
...
def train_gnn(
    ...
    if loss_name == "asymmetric":
        criterion = CombinedASLLoss(lambda_c=lambda_c, lambda_p=lambda_p)
    else:
...
def evaluate_gnn(model, data, n_mc: int = 20) -> dict:
    """
    Evaluate GNN on test set using Adaptive Uncertainty-Gated Threshold (AUGT).

    Process
    -------
    1. Run MC Dropout (n_mc passes) on val set → mean probs + uncertainty (std).
    2. Fit AUGT thresholds (base_low, base_high, k) on val set using uncertainty.
    3. Run MC Dropout on test set → mean probs + uncertainty.
    4. Apply uncertainty-aware AUGT decision rule on test set → metrics including AURC.
    5. Fall back to static threshold if val set has only one class.

    Parameters
    ----------
    model : Trained GNN (ElliGAT or compatible).
    data  : Full-graph PyG Data object.
    n_mc  : Number of MC Dropout passes (default 20 for speed; use 30 in final eval).

    Returns
    -------
    dict matching the keys used by compute_metrics() plus AUGT/AURC keys.
    """
    from src.uncertainty import fit_augt_thresholds, evaluate_augt

    # ── Val-set MC Dropout to fit AUGT thresholds ────────────────────────
    if hasattr(model, "mc_dropout_forward"):
        logits_mc_val = model.mc_dropout_forward(data, n_mc)
        probs_mc_val  = torch.sigmoid(logits_mc_val)[:, data.val_mask].cpu().numpy()
    else:
        model.train()
        with torch.no_grad():
            probs_mc_val = np.stack([
                torch.sigmoid(model(data)[data.val_mask]).cpu().numpy()
                for _ in range(n_mc)
            ])
        model.eval()

    val_probs = probs_mc_val.mean(axis=0)
    val_unc   = probs_mc_val.std(axis=0)
    val_y     = data.y[data.val_mask].cpu().numpy()

    use_augt = len(np.unique(val_y)) > 1
    if use_augt:
        base_low, base_high, k = fit_augt_thresholds(val_y, val_probs, val_unc)
    else:
        base_low, base_high, k = 0.30, 0.70, 0.20

    # ── Test-set MC Dropout ───────────────────────────────────────────────
    if hasattr(model, "mc_dropout_forward"):
        logits_mc_test = model.mc_dropout_forward(data, n_mc)
        probs_mc_test  = torch.sigmoid(logits_mc_test)[:, data.test_mask].cpu().numpy()
    else:
        model.train()
        with torch.no_grad():
            probs_mc_test = np.stack([
                torch.sigmoid(model(data)[data.test_mask]).cpu().numpy()
                for _ in range(n_mc)
            ])
        model.eval()

    test_probs = probs_mc_test.mean(axis=0)
    test_unc   = probs_mc_test.std(axis=0)
    test_y     = data.y[data.test_mask].cpu().numpy()

    if use_augt and len(np.unique(test_y)) > 1:
        result = evaluate_augt(test_y, test_probs, test_probs, test_unc,
                               base_low, base_high, k)
    else:
        # Fallback: standard evaluation
        result = compute_metrics(test_y, test_probs)
